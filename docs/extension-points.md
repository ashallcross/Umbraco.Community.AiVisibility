...
- **`IExclusionEvaluator`** — shared shape for the per-doctype + per-page `excludeFromLlmExports` boolean used by `MarkdownController` + `AcceptHeaderNegotiationMiddleware` + the Razor TagHelpers. The default evaluator also consults `IPublicAccessService.IsProtected(...)` so member-protected content is excluded from every Markdown surface. See [`data-attributes.md`](data-attributes.md).
