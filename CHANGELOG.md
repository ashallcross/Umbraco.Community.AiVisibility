...
- **`AiVisibility:RenderStrategy:Mode`** — new top-level config key (`Auto` / `Razor` / `Loopback`, default `Auto`). Controls how the package renders a page for Markdown extraction. Backwards-compatible: clean sites that worked under v1.0.x see no change under the new `Auto` default. See [`docs/configuration.md`](docs/configuration.md#aivisibilityrenderstrategy--page-rendering-for-hijacked-content) for the full key reference and [`docs/getting-started.md`](docs/getting-started.md) § Rendering strategies for hijacked pages for adopter guidance on when to pin each mode.
- **`AiVisibility:RenderStrategy:LoopbackBaseUrl`** — new optional config key (`string?`, default `null`). Escape hatch for the loopback transport target when `IServerAddressesFeature` autodetection fails (containers, reverse-proxy topologies, multi-tenant hosting). Format is scheme + authority only (e.g. `http://127.0.0.1:8080`). Malformed values surface lazily as `InvalidOperationException` at the first loopback render — adopters pinned to `Mode = Razor` with a typo'd value still boot clean.
- **HTTP loopback render path** — for hijacked pages where the controller binds a custom non-`IPublishedContent` view model, the package now issues an in-process HTTP loopback request to the canonical published URL, parses the resulting HTML, and converts to Markdown. Decouples TCP transport target from the outbound `Host:` header so multi-site `IDomainService` resolution sees the original published host. Loopback uses an `IHttpClientFactory` named client `"AiVisibility.Loopback"` with `AllowAutoRedirect = false` and a recursion guard (`X-AiVisibility-Loopback: 1` header + loopback-IP check) for defence in depth.
- **`Auto` fallback decision cache** — per-`(ContentTypeAlias, TemplateAlias)` in-memory store; the first failed Razor attempt per tuple per process logs a one-time `Warning` (`PageRenderer: Razor → Loopback fallback fired for {Alias} {ContentKey} {Path}`); subsequent renders skip Razor for that tuple until app restart. Tuple key (not doctype-only) ensures alternate templates that render cleanly aren't over-cached as hijacked.
- **Mode is read live** via `IOptionsMonitor<AiVisibilitySettings>` at the top of every render call — flipping `AiVisibility:RenderStrategy:Mode` in `appsettings.json` takes effect on the next render with no restart.

### Fixed

- **Member-protected pages no longer leak login-redirect HTML into Markdown surfaces.** Pages flagged via Umbraco's Public Access feature (member-restricted access) now return `404` from `GET /{path}.md` and are omitted from `/llms.txt` and `/llms-full.txt` regardless of the active `RenderStrategy:Mode`. Latent v1.0.x bug — the previous behaviour rendered the login-redirect HTML as Markdown content for any AI crawler that hit a protected URL. The fix extends the existing `IExclusionEvaluator` chain via `IPublicAccessService.IsProtected(content.Path)`, so it applies to the Razor strategy too (not just the new loopback path).
- **`RenderStrategy:Mode = Auto` — non-`ModelBindingException` Razor failures are now cached.** Previously the Auto strategy only cached the `(ContentTypeAlias, TemplateAlias)` decision on `ModelBindingException` (the hijack trigger); every other Razor failure (no template, view not found, structural doctype with no renderable view, Razor compilation issue) propagated without caching. On sites where `/llms-full.txt` iterated many pages sharing the same uncacheable `(doctype, template)` tuple, this caused the Razor render to be re-attempted on every iteration with the same per-tuple log line each time — observed at 1225 retries for 11 unique tuples on a representative agency-built site. The cache now stores two decision types per tuple: hijacked (try Loopback) and Razor-permanently-failed (skip both Razor and Loopback, return the failure). Subsequent renders of an already-failed tuple short-circuit with no retry — the one-time warning log shifts from per-render to per-tuple-per-process.

### Migration from v1.0.x

- No action required. The new `AiVisibility:RenderStrategy` config block is optional; the default `Mode = Auto` preserves v1.0.x behaviour on clean Razor-template sites.
- If you previously hit `ModelBindingException` on hijacked pages and had AI Visibility's `.md` route disabled or producing empty Markdown, you can now leave `Mode = Auto` (the package will fall back to loopback on hijacks) or pin `Mode = Loopback` to skip the Razor attempt entirely.
- If your site uses Umbraco's Public Access feature for member-restricted content, the member-protected exclusion fix takes effect automatically — previously-leaked login-redirect HTML stops rendering as Markdown content.
